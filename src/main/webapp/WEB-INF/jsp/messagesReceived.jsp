...
<html>
<%
    List<MessageDTO> received = (List<MessageDTO>) request.getAttribute("received");
%>
	<head>
	    <meta charset="utf-8">
	...
	<body>
		...
		<section class="mess_choice-panel">
			...
			<a href="#" class="mess_choice-button">RECEIVED</a>
			<a href="sent" class="mess_choice-button">SENT</a>
		</section>
		<section class="main-section mess-main-section">
			<div class="mess_main">
                <table id="mess_table">
					...
					<%! int id = 0;%>
                    <%for(MessageDTO message:received){%>
					<%
						id += 1;
						String messR_source = message.getMessageText();
						String messR_escaped = HtmlUtils.htmlEscape(messR_source);
					%>
                    <tr onclick="equalHTML( 'MSID_<%=id%>', 'mess_content-box' )">
						<td id="MSID_<%=id%>" style='display: none;'><%=messR_escaped%></td>
						<td><%=message.getSender()%></td>
						<td class="mess_timestamp"><%=message.getTimeStamp()%></td>
                        <td class="mess_respond-button"><button onclick="messageRespond('mess_new-container', 'block', 'username', '<%=message.getSender()%>')">&#62;</button></td>
					</tr>
                    <%}%>
                </table>
			</div>
